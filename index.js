 ...
 * Versión: 7.1.3 (Lista de Cartera para Admin + Ensamblaje por Partes)
 */
        ...
        if (opcion === '4' || texto.includes('asesor') || texto.includes('agente') || texto.includes('humano') || texto.includes('persona')) {
            ...
            try {
                await db.collection('solicitudes_asesor').add({
                    jid, telefono, nombreCliente, motivo: 'Solicitud manual desde menú',
                    estado: 'pendiente', fecha: admin.firestore.FieldValue.serverTimestamp()
                });
                await db.collection('modo_agente').doc(jid).set({
                    activo: true, activadoPor: 'solicitud_cliente', fecha: new Date().toISOString()
                });
            } catch(e) {}
        ...
        if (texto.includes('si') || texto === 's' || texto.includes('sí') || texto.includes('confirmo') || texto.includes('correcto')) {
            try {
                await db.collection('cotizaciones').add({
                    ...
                    tipoVehiculo: sesion.tipoVehiculo, lugarRecogida: sesion.lugarRecogida,
                    horaRecogida: sesion.horaRecogida, fechaServicio: sesion.fechaServicio,
                    destino: sesion.destino, estado: 'pendiente', fecha: admin.firestore.FieldValue.serverTimestamp()
                });
            } catch(e) {}
            await eliminarSesion(jid);
            await enviar(jid, telefono, `¡Muchas gracias
...
async function aplicarPago(jid, telefono, servicio, pago) {
    ...
    try {
        await db.collection('services').doc(servicio.id).update({
            ...
            ultimoPago: {
                valor: valorPago, numeroTransaccion: pago.numeroTransaccion || 'N/A',
                bancoOrigen: pago.bancoOrigen || 'N/A', bancoDestino: pago.bancoDestino || 'N/A',
        ...
        await db.collection('pagos_aplicados').add({
            servicioId: servicio.id, consecutivo: servicio.consecutivo,
            clienteNombre: servicio.clienteNombre || servicio.cliente,
            telefonoCliente: servicio.telefonoCliente, valorPago,
            numeroTransaccion: pago.numeroTransaccion || 'N/A',
            bancoOrigen: pago.bancoOrigen || 'N/A', bancoDestino: pago.bancoDestino || 'N/A',
            saldoAnterior: saldoActual, saldoNuevo: nuevoSaldo, estadoPago,
            fecha: admin.firestore.FieldValue.serverTimestamp()
        });
        ...
        if (estadoPago === 'Pagado') {
            ...
            await db.collection('pagos_pendientes_correo').add({
                servicioId: servicio.id, emailCliente: servicio.emailCliente,
                consecutivo: servicio.consecutivo, pendiente: true,
                fecha: admin.firestore.FieldValue.serverTimestamp()
            });
        } else {
...
async function connectToWhatsApp() {
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        if (msg.key.fromMe) {
            try {
                await db.collection('modo_agente').doc(jid).set({
                    activo: true, activadoPor: 'respuesta_directa_whatsapp', fecha: new Date().toISOString()
                });
            } catch(e) {}
            return;
        }
...
Si no es un comprobante de pago, retorna esComprobante: false.` },
                ...
                console.log(`[Nova] 💰 Comprobante: $${pago.valor} - Ref: ${pago.numeroTransaccion}`);
                ...
                if (esAdmin) {
                    ...
                    } catch (err) { listaCartera = '\n\n_(No se pudo cargar la cartera pendiente)_'; }
                    ...
                    if (serviciosSnap.empty) {
                        await enviar(jid, telefono, `Recibimos tu comprobante de $${pago.valor?.toLocaleString('es-CO')} 😊\n\nNo encontré servicios pendientes de pago asociados a tu número. Un asesor te confirmará. ⏱️`);
                        return;
                    }
                    ...
                    for (const doc of serviciosSnap.docs) {
                        ...
                        if (Math.abs(saldo - pago.valor) < saldo * 0.05 || Math.abs(valorServicio - pago.valor) < valorServicio * 0.05) {
                            servicioMatch = { id: doc.id, ...s }; break;
                        }
                    }
                ...
                await enviar(jid, telefono, `Recibimos tu imagen 📎\n\nNo pude procesar el comprobante automáticamente. Un asesor lo revisará. ⏱️`);
            }
            return;
        }
...
app.post('/send-file', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        let message = mimeType.startsWith('image/') 
            ? { image: buffer, caption: caption || '', mimetype: mimeType }
            : { document: buffer, mimetype: mimeType, fileName: fileName || 'archivo', caption: caption || '' };
